...
function CondorIWDs() {
	JobIDs=`condor_q --nobatch| grep $USER | awk '{print $1}' | cut -d. -f1 | uniq`
	for JobID in $JobIDs
	do
   	a=`condor_q -l $JobID | grep Iwd | head -n 1 | awk '{print $3}' | xargs basename`
		b=`condor_q --nobatch $JobID | grep " R " | wc -l`
		c=`condor_q --nobatch $JobID | grep " I " | wc -l`
...
function DoStatus() {
	condor_q --nobatch -submitter $USER | tail -n 1
	thisJobN=""
	if [ $1 ]; then
	...
	nAllJobs=`condor_q --nobatch $thisJobN | grep $USER | grep runCMSRUN.sh | wc -l`
	nIdleJobs=`condor_q --nobatch $thisJobN | grep $USER | grep " I " | grep runCMSRUN.sh | wc -l`
	condorIDs=`condor_q --nobatch $thisJobN | grep $USER | grep " R " | grep runCMSRUN.sh | awk '{print $1}'`
	total=0
...
function argvPar {
   ...
   do
      ...
      if [ "${arg1}" == "-mergeHist"  ]; then doMergeHist=1; MyMergeHistDir="${arg2}" ; continue; fi
...
###start ycyang1
	DataString=${DataString/condor_/}
	DataString=${DataString/_condorOutList/}
###end ycyang1 
...
outNameFile.close()
EOF
	echo "python ${MakePSetFile} ${MyCFG} ${MyCMSARG} >& ${MakePSetFile}.log"
...
EOF
   ...
   if [ -d $CMSSW_BASE/lib ]; then ln -s $CMSSW_BASE/lib . ; fi
   if [ -d $CMSSW_BASE/biglib ]; then ln -s $CMSSW_BASE/biglib . ; fi
	if [ -d $CMSSW_BASE/external ]; then ln -s $CMSSW_BASE/external . ; fi
###################
	cDir=$PWD
...
requirements = (machine != "wn3010.sdfarm.kr")
...
if [ "${isRunning}" != "" ]; then
	...
	echo "$isRunning" | awk '{print $2}' | xargs condor_q --nobatch 
...
nLog=`ls -l $dir/condorLog/condorLog*.log | wc -l`
...
function MergeHist() {
	...
	if [ ! -f $outName ]; then
		...
		if [ "$numSRM" == "0" ]; then
			...
			haddNFile=`cat $dirName/condorOut/*.srm | wc -l`
			if [ $haddNFile -le 500 ]; then 
				hadd -T $outName `cat $dirName/condorOut/*.srm` >& /dev/null
			else
				cat $dirName/condorOut/*.srm > mHistTotalList.txt
				split -d -l 500 mHistTotalList.txt mergeHistList_
				for mListFile in `ls -1 mergeHistList_*`
				do
					hadd -T `basename ${mListFile}`.root `cat $mListFile` >& /dev/null
				done
				hadd -T $outName mergeHistList_*.root >& /dev/null
				rm -rf mHistTotalList.txt mergeHistList_* mergeHistList_*.root
			fi
		fi
	fi
